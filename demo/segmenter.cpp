...
DEFINE_int32(camera_id, -1, "Use usb video");
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_image == "" & FLAGS_image_list == "" & FLAGS_camera_id == -1) {
    std::cerr << "--image or --image_list or --camera_id need to be defined" << std::endl;
    return -1;
  }
  ...
  if (FLAGS_image_list != "") {
    ...
    cv::Mat dst;
    cv::addWeighted(im,1,vis_img,0.6,0,dst);
    ...
    while (true){
      ...
      // Visualize results
      cv::Mat vis_img = PaddleX::Visualize(im, result, model.labels);
      cv::addWeighted(im,1,vis_img,0.3,3,dst);
      cv::imshow("result", dst);
      result.clear();
      std::cout << "Visualized Success " << std::endl;
      cv::waitKey(1);
    }
  }
  return 0;
